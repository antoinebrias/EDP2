...
if mdlstruct.is_value_function_log
...
mOut=min(z); %%%%%% CHANGE 08/07/22
sdOut=max(z)-mOut;


zOut=(z-mOut)./(sdOut+eps);

switch mdlstruct.value_function_type
    case 'gp'
        ...
        if isfield(value_function,'gp')
         ...
         value_function.gp.is_log = mdlstruct.is_value_function_log;
...
%         value_function.gp.is_log=is_log;
        ...
        value_function.gp.is_value_function=1;
    ...
    case 'nn'
        % work only for 2d
         ...
         for i=1:n_dim_output
        value_function.nn.nn_handle{i} = fitcknn(x,zOut(:,i),'NumNeighbors',1,'Standardize',1);
         end
         
        value_function.nn.is_log = mdlstruct.is_value_function_log;
        value_function.nn.ind_available_var=ind_available_var;
        value_function.nn.n_dim=n_dim_output;
        value_function.nn.mIn=mIn;
        value_function.nn.sdIn=sdIn;
        value_function.nn.mOut=mOut;
        value_function.nn.sdOut=sdOut;
        value_function.nn_model =  @(x) post_nn(x,0,value_function.nn);
       
        
end

value_function.value_function_type=mdlstruct.value_function_type;
end
